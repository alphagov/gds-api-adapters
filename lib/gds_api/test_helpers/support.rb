module GdsApi
  module TestHelpers
    module Support
      ...
      def stub_support_transaction_feedback_creation(request_details = nil)
        post_stub = stub_http_request(:post, "#{SUPPORT_ENDPOINT}/transaction_feedback")
        post_stub.with(:body => {"transaction_feedback" => request_details}) unless request_details.nil?
        post_stub.to_return(:status => 201)
      end
