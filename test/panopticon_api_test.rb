...
class PanopticonApiTest < MiniTest::Unit::TestCase
  ...
  def api
    GdsApi::Panopticon.new(ENDPOINT)
  end

  def test_given_a_slug__should_fetch_artefact_from_panopticon
    slug = 'an-artefact'
    artefact_json = { name: 'An artefact' }.to_json
    stub_request(:get, "#{ENDPOINT}/artefacts/#{slug}.json").to_return(body: artefact_json)
  ...
  def test_should_construct_correct_url_for_a_slug
    assert_equal "#{ENDPOINT}/artefacts/slug.json", api.url_for_slug('slug')
  end
