...
describe "GdsApi::PublishingApi#get_content pact tests" do
  ...
  it "responds with 200 and the published content item when requesting from the live content store" do
    publishing_api
    ...
    api_client.get_content(content_id, content_store: :live)
  end

  it "responds with 200 and the published content item when requesting no specific content_store" do
    publishing_api
      .given("a content item exists in with a superseded version with content_id: #{content_id}")
      .upon_receiving("a request to return the content item")
      .with(
